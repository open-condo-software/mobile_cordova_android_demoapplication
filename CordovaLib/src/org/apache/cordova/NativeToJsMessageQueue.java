...
public class NativeToJsMessageQueue {
    ...
    public void reset() {
        synchronized (this) {
            queue.clear();
            Log.d("CORDOVA","reset() queue.clear()");
            setBridgeMode(-1);
        }
    }
    ...
    public String popAndEncode(boolean fromOnlineEvent) {
        synchronized (this) {
            ...
            for (int i = 0; i < numMessagesToSend; ++i) {
                ...
                Log.d("CORDOVA", "popAndEncode JsMessage" + message.jsPayloadOrCallbackId);
    ...
    public void addPluginResult(PluginResult result, String callbackId) {
        Log.d("CORDOVA","sendPluginResult callbackId"+callbackId);
    ...
    private void enqueueMessage(JsMessage message) {
        synchronized (this) {
            Log.d("CORDOVA", "enqueueMessage "+message.jsPayloadOrCallbackId + "paused:" +paused);

            if (activeBridgeMode == null) {
                Log.d("CORDOVA", "Dropping Native->JS message due to disabled bridge");
                return;
            }
            queue.add(message);
            if (!paused) {
    ...
    public void setPaused(boolean value) {
        ...
        if (!value) {
            synchronized (this) {
                Log.d("CORDOVA","setPaused queue "+queue.toString());

                if (!queue.isEmpty() && activeBridgeMode != null) {
                    Log.d("CORDOVA","setPaused queue is not empty"+queue.toString());
                    activeBridgeMode.onNativeToJsMessageAvailable(this);
                }
            }
        }
        ...
        public void onNativeToJsMessageAvailable(final NativeToJsMessageQueue queue) {
            cordova.getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    if (js != null) {
                        Log.d("CORDOVA","evaluateJavascript "+js);
            ...
            delegate.runOnUiThread(new Runnable() {
                public void run() {
                    if (!queue.isEmpty()) {
                        ...
                        Log.d("CORDOVA","OnlineEventsBridgeMode ");
